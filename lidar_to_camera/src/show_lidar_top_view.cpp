...
void showLidarTopview()
{
    ...
    {
        float xw = (*it).x; // world position imain m with x facing forward from sensor
        ...
        {
            float distance = it->x;
            float max_distance = worldSize.height; //20m
            int green = min(255, (int)(255 * (distance/ max_distance)));
            int red = min(255, (int)(255 * (1 -(distance/ max_distance))));
