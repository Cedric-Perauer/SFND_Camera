...
void detKeypoints1()
{
    ...
    cv::Ptr<cv::FeatureDetector> detector = cv::FastFeatureDetector::create(threshold, nonmax, cv::FastFeatureDetector::TYPE_9_16);

    vector<cv::KeyPoint> kptsFAST;
    t = (double)cv::getTickCount();
    detector->detect(imgGray, kptsFAST);
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    cout << "FAST Algo with n= " << kptsFAST.size() << " keypoints in " << 1000 * t / 1.0 << " ms" << endl;
